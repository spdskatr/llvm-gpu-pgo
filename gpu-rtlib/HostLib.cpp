...
uint64_t INSTR_PROF_RAW_VERSION_VAR =
    INSTR_PROF_RAW_VERSION | VARIANT_MASK_IR_PROF;

// Device addresses for profiling data. Registered with device side by
// GPUInstrPass.
...
static void memcpyArbitraryDeviceToHost(void *hostPtr, void *proxyPtr,
...
static void debugLog() {
  ...
  // Show some data
  DEBUG_PRINTF("data:\n");
  for (char *c = reinterpret_cast<char *>(HostLoc.DataFirst);
       c < reinterpret_cast<char *>(HostLoc.DataLast); c++) {
...
template <typename T> static size_t distanceInBytes(T *start, T *end) {
...
static void fixRelativePositions() {
  ...
  uintptr_t HostCounterRel =
      reinterpret_cast<uintptr_t>(HostLoc.CountersFirst) -
      reinterpret_cast<uintptr_t>(HostLoc.DataFirst);
  uintptr_t DeviceCounterRel =
      reinterpret_cast<uintptr_t>(DeviceLoc.CountersFirst) -
...
static void allocateLocs() {
  ...
  HostLoc.DataFirst = static_cast<__llvm_profile_data *>(
      malloc(DataSize * sizeof(__llvm_profile_data)));
  ...
  HostLoc.CountersFirst =
      static_cast<char *>(malloc(CountersSize * sizeof(char)));
  HostLoc.CountersLast = HostLoc.CountersFirst + CountersSize;

  HIP_ASSERT(
      hipMalloc(&ProxyLoc.DataFirst, DataSize * sizeof(__llvm_profile_data)));
