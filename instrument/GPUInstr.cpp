...
static cl::opt<bool> DisableWarpBalloting{
    "no-warp-ballot", cl::init(false), cl::Hidden,
    cl::desc("Disable the warp balloting transfomation")};
...
PreservedAnalyses GPUInstrPass::run(Module &M, ModuleAnalysisManager &AM) {
  ...
  if (M.getTargetTriple() == "amdgcn-amd-amdhsa") {
    ...
    ModuleInlinerWrapperPass MIWP{
        IP, true,
        InlineContext{.LTOPhase = ThinOrFullLTOPhase::ThinLTOPreLink,
    ...
    if (!UseProfilePath.empty()) {
      errs() << "Using " << UseProfilePath
             << " as profile file for PGOInstrumentationUse\n";
      MPM.addPass(PGOInstrumentationUse{UseProfilePath});
      MPM.addPass(RequireAnalysisPass<ProfileSummaryAnalysis, Module>{});
    } else {
      errs() << "Running PGO instrumentation on module "
             << M.getModuleIdentifier() << " with device target "
             << M.getTargetTriple() << "\n";
      // Insert intrinsics
      ...
      if (!DisableWarpBalloting) {
        ...
        MPM.addPass(
            createModuleToFunctionPassAdaptor(IncrementToWarpBallotPass{}));
      }
      ...
      // Lower intrinsics
      // NOTE: LLVM was modified to change bitcasts to addrspace casts
      MPM.addPass(InstrProfiling{InstrProfOptions{.NoRedZone = false,
