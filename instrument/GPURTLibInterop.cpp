...
static void insertRegisterVar(Module &M) {
  ...
  if (!RegisterGlobalsF) {
    errs() << "Module " << M.getName()
           << " is probably not a HIP host module, skipping.\n";
  } else {
    auto *RegisterVarF = M.getFunction(HIP_REGISTER_VAR_NAME);

    // (&*p): Dereference the iterator and then reference the result to get the
    // pointer.
    ...
    auto *LocVar =
        new GlobalVariable(M, PtrTy, false, llvm::GlobalValue::ExternalLinkage,
    ...
    ArrayRef<Value *> args{
        {GPUHandle, LocVar, LocNameVar, LocNameVar, ConstantInt::get(IntTy, 1),
         ConstantInt::get(SizeTy, 8), ConstantInt::get(IntTy, 0),
...
static void insertDumpAtDeregister(Module &M) {
  ...
  Function *Sync = Function::Create(FT, llvm::GlobalValue::ExternalLinkage, 0u,
                                    GPUPROF_SYNC_NAME, &M);
...
PreservedAnalyses GPURTLibInteropPass::run(Module &M,
                                           ModuleAnalysisManager &AM) {
  if (M.getTargetTriple() != "amdgcn-amd-amdhsa") {
    // Not the device target, so probably the host target. Apply host-side
    // instrumentation
