...
static void modifyIncrement(Module *M, CallInst *Call, Value *Val) {
  ...
  Function *ReadReg =
      Intrinsic::getDeclaration(M, Intrinsic::read_register, Int64Ty);
  ...
  // Get exec register
  Value *ExecReg = MetadataAsValue::get(
      M->getContext(),
  ...
  BasicBlock *After =
      B->splitBasicBlock(Call->getNextNode(), Before->getName() + ".a");
  ...
  Value *LaneId = IRB.CreateCall(
      MbcntHi, {IntNeg1, IRB.CreateCall(MbcntLo, {IntNeg1, IntZero})});
  Value *TZ =
      IRB.CreateCall(CTTZ, {Mask, ConstantInt::getBool(M->getContext(), true)});
  Value *IsLeader =
      IRB.CreateICmpEQ(LaneId, IRB.CreateIntCast(TZ, Int32Ty, false));
  ...
  LeaderB.CreateCall(IncrStep,
                     {Call->getArgOperand(0), Call->getArgOperand(1),
                      Call->getArgOperand(2), Call->getArgOperand(3), Count});
  Call->eraseFromParent();
}

PreservedAnalyses IncrementToWarpBallotPass::run(Function &F,
